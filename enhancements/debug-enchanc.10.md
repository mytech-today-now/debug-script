...
function track_custom_hooks() { add_action('all', function($hook) { global $debug_custom_hooks; if(strpos($hook, 'wp_') !== 0) { $debug_custom_hooks[$hook][] = [ 'time' => microtime(true), 'backtrace' => debug_backtrace(DEBUG_BACKTRACE_IGNORE_ARGS, 3) ]; } }); }
